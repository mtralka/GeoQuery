...
def toMap(df, user, timestamp, lat, lon):
    ...
    m = folium.Map(
        location=[lat, lon],
        API_key='pk.eyJ1IjoibXRyYWxrYSIsImEiOiJja2VjNm5hdWEwNjQ4MnZ0cH \
        lycXlndnN5In0.mfQAFUPzfGZeMht0EToJBA',
        attr='Mapbox | <strong>GeoQuery by Matthew Tralka</strong>',
        zoom_start=14,
        min_zoom=1,
        prefer_canvas=True,
        tiles="https://api.mapbox.com/styles/v1/mapbox/satellite-v8/tiles/{z}/ \
        {x}/{y}?access_token=pk.eyJ1IjoibXRyYWxrYSIsImEiOiJja2VjNm5hdWEwNjQ4MnZ0\
        cHlycXlndnN5In0.mfQAFUPzfGZeMht0EToJBA"
    )
    points = folium.FeatureGroup(name='Flickr Points')

    for ownername, title, date_taken, tags, \
        latitude, longitude, accuracy, url_l in zip(
            df['ownername'], df['title'], df['datetaken'],
            df['tags'], df['latitude'], df['longitude'],
        ...
        points.add_child(folium.CircleMarker(
            location=[latitude, longitude], fill_opacity=.7,
            fill_color='blue', radius=4, color='red', opacity=.6,
