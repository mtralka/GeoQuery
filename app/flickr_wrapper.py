...
def executeSearch(
    params, user, request_page=1, search_id=0, master=False, df=None, total_page=None
):
...
def flickrControl(raw_query, user, timestamp, task):
    ...
    while current_page <= total_page:
        ...
        time.sleep(0.5)
    task.update_state(state="IN PROGRESS", meta={"status": "saving flickr results..."})
    ...
    task.update_state(state="IN PROGRESS", meta={"status": "making flickr maps..."})
